...
class ProgramsApiTest extends TestCase
{
    ...
    public function testGetProgramsList()
    {
        $response = $this->request('GET', 'programs.php', $this->studentToken);
        $this->assertEquals(200, $response['status']);
        $this->assertArrayHasKey('data', $response['body']);
    }

    public function testStudentCannotCreateProgram()
    {
        $response = $this->request('POST', 'programs.php', $this->studentToken, [
            'name' => 'Hack Attempt',
            'code' => 'HCK'
        ]);
    ...
    public function testAdminCreateProgram()
    {
        $uniqCode = 'TS' . rand(10, 99);
        $response = $this->request('POST', 'programs.php', $this->adminToken, [
