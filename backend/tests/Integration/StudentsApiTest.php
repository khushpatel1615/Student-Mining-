<?php

namespace Tests\Integration;

use PHPUnit\Framework\TestCase;

class StudentsApiTest extends TestCase
{
    private string $baseUrl;
    private string $adminToken;
    private string $studentToken;

    protected function setUp(): void
    {
        $this->baseUrl = getenv('TEST_BASE_URL') ?: 'http://localhost/backend/api/';

        require_once __DIR__ . '/../../includes/jwt.php';
        $this->adminToken = generateToken(1, 'admin@college.edu', 'admin', 'Admin User');
        $this->studentToken = generateToken(3, 'student@college.edu', 'student', 'Student User');
    }

    private function request(string $method, string $endpoint, ?string $token = null, array $data = []): array
    {
        $url = rtrim($this->baseUrl, '/') . '/' . ltrim($endpoint, '/');

        $headers = "Content-Type: application/json\r\n";
        if ($token) {
            $headers .= "Authorization: Bearer $token\r\n";
        }

        $options = [
            'http' => [
                'header' => $headers,
                'method' => $method,
                'content' => in_array($method, ['POST', 'PUT', 'DELETE']) && !empty($data) ? json_encode($data) : null,
                'ignore_errors' => true
            ]
        ];

        $context = stream_context_create($options);
        $response = file_get_contents($url, false, $context);

        $status = 0;
        if (isset($http_response_header[0]) && preg_match('#HTTP/\d+\.\d+ (\d+)#', $http_response_header[0], $matches)) {
            $status = (int) $matches[1];
        }

        return [
            'status' => $status,
            'body' => json_decode($response, true)
        ];
    }

    public function testGetStudentsList()
    {
        $response = $this->request('GET', 'students.php?page=1&limit=10', $this->adminToken);
        $this->assertEquals(200, $response['status']);
        $this->assertTrue($response['body']['success'] ?? false);
        $this->assertArrayHasKey('data', $response['body']);
        $this->assertArrayHasKey('pagination', $response['body']);
    }

    public function testGetStudentById()
    {
        // Try getting ID 3, assuming seeded
        $response = $this->request('GET', 'students.php?id=3', $this->adminToken);
        $this->assertEquals(200, $response['status']);
    }

    public function testStudentCannotGetOtherStudents()
    {
        $response = $this->request('GET', 'students.php?id=4', $this->studentToken);
        $this->assertEquals(403, $response['status']);
    }

    public function testStudentCannotCreateOrUpdate()
    {
        $response = $this->request('POST', 'students.php', $this->studentToken, [
            'full_name' => 'Hack Attempt'
        ]);
        $this->assertEquals(403, $response['status']);
    }

    public function testAdminCreateStudent()
    {
        $uniqid = uniqid();
        $response = $this->request('POST', 'students.php', $this->adminToken, [
            'full_name' => 'Integration Test Student',
            'email' => "integration_$uniqid@college.edu",
            'student_id' => "INT_$uniqid",
            'program_id' => 1
        ]);

        // Either 201 Created or 200 OK
        $this->assertContains($response['status'], [200, 201]);
    }
}
