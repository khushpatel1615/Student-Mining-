...
class TeachersApiTest extends TestCase
{
    ...
    {
        $response = $this->request('GET', 'teachers.php', $this->adminToken);
        $this->assertEquals(200, $response['status']);
        $this->assertTrue($response['body']['success'] ?? false);
        $this->assertArrayHasKey('data', $response['body']);
    }

    public function testTeacherCannotCreate()
    {
        $response = $this->request('POST', 'teachers.php', $this->teacherToken, [
            'full_name' => 'Hack Attempt'
        ]);
        $this->assertEquals(403, $response['status']);
    }

    public function testTeacherCanGetMySubjects()
    {
        $response = $this->request('GET', 'teachers.php?action=my_subjects', $this->teacherToken);
        $this->assertEquals(200, $response['status']);
        $this->assertTrue($response['body']['success'] ?? false);
    }
}
