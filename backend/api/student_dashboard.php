...
function handleGet($pdo, $dataDir, $userId)
{
        ...
        if ($enrollment) {
            ...
            foreach ($subjectData['components'] as $comp) {
                if ($comp['marks_obtained'] !== null && $comp['max_marks'] > 0) {
                    ...
                    $componentPercent = max(0, min(100, $componentPercent));
...
{
    $stmt = $pdo->prepare("
        SELECT u.id, u.program_id, u.current_semester, p.name as program_name, p.code as program_code
        FROM users u
        LEFT JOIN programs p ON u.program_id = p.id
        WHERE u.id = ?
    ");
...
function getSubjectAttendanceFromDb($pdo, $userId, $subjectId)
{
    ...
    foreach ($rows as $row) {
        ...
        $stats['records'][] = ['date' => $row['attendance_date'], 'status' => $row['status']];
    }
...
{
    if ($attendanceRate >= 90 && $gradePercent >= 75)
        return 'High';
    if ($attendanceRate >= 75 && $gradePercent >= 60)
        return 'Medium';
    return 'Low';
}
...
function buildInsights($gradeTrend, $attendanceTrend, $riskScore, $riskLabel, $overallAttendance, $latestGrade)
{
    ...
    if (count($gradeTrend) >= 2) {
        ...
        if ($prev > 0) {
            ...
            if ($delta >= 2) {
                $insights[] = ['type' => 'grade_improvement', 'tone' => 'success', 'title' => 'Grade Improvement', 'message' => 'Your grades improved by ' . round($delta, 1) . '% since the last assessment.'];
            } elseif ($delta <= -2) {
                $insights[] = ['type' => 'grade_drop', 'tone' => 'warning', 'title' => 'Grade Dip', 'message' => 'Your grades dropped by ' . abs(round($delta, 1)) . '% since the last assessment.'];
            }
        }
    }
    ...
    if ($overallAttendance > 0) {
        if ($overallAttendance < 90) {
            $insights[] = ['type' => 'attendance_watch', 'tone' => 'warning', 'title' => 'Attendance Watch', 'message' => 'Your attendance is ' . $overallAttendance . '%. Try to stay above 90% to avoid risk.'];
        } elseif ($overallAttendance >= 95) {
            $insights[] = ['type' => 'attendance_strong', 'tone' => 'success', 'title' => 'Great Attendance', 'message' => 'Your attendance is excellent at ' . $overallAttendance . '%. Keep it up!'];
        }
    }

    if ($riskScore >= 70) {
        $insights[] = ['type' => 'risk_high', 'tone' => 'danger', 'title' => 'High Risk Alert', 'message' => 'Your risk score is ' . $riskScore . '. Consider scheduling a check-in with your advisor.'];
    } elseif ($riskScore >= 40) {
        $insights[] = ['type' => 'risk_moderate', 'tone' => 'warning', 'title' => 'Moderate Risk', 'message' => 'Your risk score is ' . $riskScore . '. Focus on consistent attendance and assessments.'];
    } else {
        $insights[] = ['type' => 'risk_safe', 'tone' => 'success', 'title' => 'Low Risk', 'message' => 'You are in the safe zone. Keep your current momentum.'];
    }

    if (empty($gradeTrend)) {
        $insights[] = ['type' => 'no_grades', 'tone' => 'neutral', 'title' => 'Grades Pending', 'message' => 'No graded assessments yet. Insights will appear after your first marks are uploaded.'];
    }
    return $insights;
}
