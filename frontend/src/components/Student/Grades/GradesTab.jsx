...
const GradesTab = ({ selectedSemester }) => {
    ...
    const fetchGrades = async () => {
        ...
        setError('');
        try {
            const { data, error: fetchError } = await fetchStudentGrades(user.id);
            if (fetchError) {
            ...
            setError(error.message || 'Failed to fetch grades');
            setGrades([]);
        } finally {
            setLoading(false);
        }
    ...
    const currentSubjects = useMemo(() => {
        ...
        if (selectedSemester) {
            filtered = filtered.filter(subject => Number(subject.semester) === Number(selectedSemester));
        } else if (user?.current_semester) {
            filtered = filtered.filter(subject => Number(subject.semester) === Number(user.current_semester));
        } else {
            filtered = filtered.filter(subject => {
                const status = String(subject.status || subject.enrollment_status || '').toLowerCase();
    ...
    if (error) return <div className="loading-state" style={{ color: '#b91c1c' }}>{error}</div>;
